DROP TABLE IF EXISTS dune_user_generated.balancer_v2_view_liquidity_draft;

CREATE TABLE dune_user_generated.balancer_v2_view_liquidity_draft (
    day timestamp,
    pool_id bytea,
    pool_symbol text,
    token_address bytea,
...
WITH pool_labels AS (
        ...
        FROM (
            ...
            FROM labels.labels
            WHERE "type" IN ('balancer_v2_pool')
            GROUP BY 1, 2
        ) l
        WHERE num = 1
    ),
    ...
    internal_changes AS (
        SELECT date_trunc('day', evt_block_time) AS day, NULL::bytea AS pool, token, SUM(COALESCE(delta, 0)) AS delta 
    ...
    pool_liquidity_estimates AS (
        ...
        LEFT JOIN pools_tokens_weights w
        ON b.pool = w.pool_id
    ...
    balancer_liquidity AS (
        ...
        LEFT JOIN cumulative_usd_balance c ON c.day = b.day AND c.pool = b.pool
        LEFT JOIN pools_tokens_weights w ON b.pool = w.pool_id AND w.token_address = c.token
...
INSERT INTO dune_user_generated.balancer_v2_view_liquidity_draft
