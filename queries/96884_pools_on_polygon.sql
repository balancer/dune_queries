WITH labels AS (
    ...
    cumulative_usd_balance AS (
        ...
        LEFT JOIN dune_user_generated.prices_usd p1 ON p1.minute = c.day AND p1.contract_address = b.token
    ),
   ...
   lps_shares AS (
        SELECT c.day, c.lp, c.pool,
            CASE WHEN total_bpt > 0 THEN c.amount_bpt/d.total_bpt ELSE 0 END AS share
    ...
    pools AS (
        SELECT
            ...
            t.tvl,
            CASE WHEN '{{1. LP address}}' = 'All' THEN NULL ELSE share END AS share,
            s.lp
        FROM lps_shares s
        INNER JOIN last_tvl t ON t.pool = s.pool
        WHERE lp = CONCAT('\', SUBSTRING('{{1. LP address}}', 2))::bytea
        OR '{{1. LP address}}' = 'All'
    )
...
SELECT 
    ...
    CONCAT('<a target="_blank" href="https://polygon.balancer.fi/#/pool/0', SUBSTRING(r."poolId"::text, 2), '">view pool</a>') AS pool,
    CONCAT('<a target="_blank" href="https://polygonscan.io/address/0', SUBSTRING(pool::text, 2, 42), '">0', SUBSTRING(pool::text, 2, 42), '</a>') AS polygonscan
FROM pools p
