WITH lbp_info AS (
    ...
    days AS (
        SELECT generate_series('2020-01-01'::timestamp, date_trunc('day', NOW()), '1 hour') AS day
), 
...
SELECT  b.day, l.token_symbol, COUNT(DISTINCT address) AS holders
FROM balance_all_days b
INNER JOIN lbp_info l ON l.token_sold = b.token_address
WHERE balance > 0 AND b.day BETWEEN l.initial_time AND l.final_time
GROUP BY 1, 2