WITH prices AS (
    ...
    last_liquidity_volume AS (
        SELECT date_trunc('week', day) AS week, token, symbol, liquidity, volume,
        CASE WHEN '{{Token}}' <> 'none' THEN (CASE WHEN symbol = '{{Token}}' THEN symbol
        ELSE 'Others' END) ELSE symbol END AS class
        FROM liquidity_volume l
        WHERE date_trunc('week', day) = date_trunc('week', CURRENT_DATE - '1 week'::interval)
    )
    
SELECT week, token, symbol, class, AVG(liquidity) AS liquidity, AVG(volume) AS volume FROM last_liquidity_volume GROUP BY 1,2,3,4