WITH labels AS (
    ...
    fees AS (
     ...
     last_liquidity_volume AS (
        SELECT date_trunc('day', day) AS day, l.pool, l.symbol, liquidity, volume,
        f.swap_fee::text,
        ROUND(f.swap_fee::numeric, {{Swap fee decimals}})::text AS fee_class
        FROM liquidity_volume l INNER JOIN fees f ON l.pool = f.pool
        WHERE date_trunc('day', day) = date_trunc('day', CURRENT_DATE - '1 day'::interval)
    )

SELECT
    CASE WHEN '{{Fee value}}' <> 'none' THEN (CASE WHEN fee_class = '{{Fee value}}' THEN fee_class
    ELSE 'Others' END) ELSE fee_class END AS class,
    day, pool, symbol, AVG(liquidity) AS liquidity, AVG(volume) AS volume
