...
tvl_v1 AS (
    ...
    FROM
        balancer."view_pools_liquidity" p
    WHERE
        pool <> '\xBA12222222228d8Ba445958a75a0704d566BF2C8'
        AND DAY <= '{{3. End date}}'
    GROUP BY
        1,
        2
...
SELECT
    ...
    v1.tvl AS "V1",
    v2.tvl AS "V2",
    COALESCE(v1.tvl, 0) + COALESCE(v2.tvl, 0) AS "Total"
FROM
    tvl_v1 v1 FULL
    OUTER JOIN tvl_v2 v2 ON v1.day = v2.day
WHERE
    v1.day >= '{{2. Start date}}'