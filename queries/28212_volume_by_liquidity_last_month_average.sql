WITH labels AS (
    ...
    last_liquidity_volume AS (
        SELECT date_trunc('month', day) AS month, pool, symbol, liquidity, volume 
        FROM liquidity_volume
        WHERE date_trunc('month', day) = date_trunc('month', CURRENT_DATE - '1 month'::interval)
    )
    
SELECT month, pool, symbol, AVG(liquidity) AS liquidity, AVG(volume) AS volume FROM last_liquidity_volume GROUP BY 1,2,3