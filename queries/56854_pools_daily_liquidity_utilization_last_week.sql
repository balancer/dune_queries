...
volume AS (
    ...
    FROM
        dex.trades t
        INNER JOIN labels l ON t.exchange_contract_address :: text = l.address
...
liquidity AS (
    ...
    FROM
        ...
        INNER JOIN labels l ON b.pool :: text = l.address
        AND l.version = '1'
    WHERE
        date_trunc('day', DAY) > date_trunc('day', CURRENT_DATE - '1 week' :: INTERVAL)
        AND l.label = '{{Pool}}'
    GROUP BY
        1,
        2
