WITH prices AS (
    ...
    v2_cumulative_usd_balance AS (
        ...
        FROM calendar c
        LEFT JOIN v2_cumulative_balance b ON b.day <= c.day AND c.day < b.day_of_next_change
    ...
    v2_estimated_pool_liquidity as (
        ...
        FROM v2_cumulative_usd_balance
        GROUP BY 1, 2, 3
    ),
    
    v1_estimated_token_liquidity AS (
        SELECT token_address, day, SUM(liquidity * normalized_weight) AS liquidity
        FROM balancer."view_pools_liquidity" b
        INNER JOIN balancer."view_pools_tokens_weights" w ON b.pool = w.pool_id
        -- INNER JOIN erc20.tokens t ON w.token_address = t.contract_address
        WHERE token_address = (SELECT contract_address
                                FROM erc20.tokens WHERE symbol = '{{Token}}')
        GROUP BY 1, 2
    ),
    ...
    swaps AS (
        ...
        AND token_a_address = (SELECT contract_address
                                FROM erc20.tokens WHERE symbol = '{{Token}}')
        GROUP BY 1, 2, 3
        ...
        WHERE project = 'Balancer'
        AND date_trunc('day', block_time) > date_trunc('day', CURRENT_DATE - '1 week'::interval)
        AND token_b_address = (SELECT contract_address
                                FROM erc20.tokens WHERE symbol = '{{Token}}')
        GROUP BY 1, 2, 3
    ),
    ...
    liquidity AS (
        ...
        FROM v1_estimated_token_liquidity b
        ...
        FROM v2_estimated_pool_liquidity
        WHERE date_trunc('day', day) > date_trunc('day', CURRENT_DATE - '1 week'::interval)
        AND token = (SELECT contract_address FROM erc20.tokens WHERE symbol = '{{Token}}')
        GROUP BY 1, 2
    ),
