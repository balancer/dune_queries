...
pools AS (
    SELECT
        '\xfae2809935233d4bfe8a56c2355c4a2e7d1fff1a' :: bytea AS pools -- FROM balancer."BFactory_evt_LOG_NEW_POOL"
),
joins AS (
    SELECT
...
lp_share_by_pool AS (
    SELECT
        ...
        c.lp,
        c.pool,
        c.amount_bpt / d.total_bpt AS SHARE
    FROM
...
daily_lp_tvl AS (
    SELECT
        p.day,
        p.lp,
        SUM(p.share * d.liquidity) AS tvl
    FROM
        lp_share_by_pool p
        INNER JOIN estimated_pool_liquidity d ON d.day = p.day
        AND d.pool = p.pool
    GROUP BY
        1,
        2
...
SELECT
    p.day,
    COALESCE(t.lp :: text, 'Others') AS lp,
    SUM(p.tvl) AS "TVL"
FROM
    daily_lp_tvl p
