WITH daily_minted AS (
    ...
    cumulative_minted AS (
        SELECT
            day,
            LEAD(day, 1, now()) OVER (ORDER BY day) AS day_of_next_change,
            SUM(value) OVER (ORDER BY day) AS cumulative_value
        FROM daily_minted
    ),

    calendar AS (
        SELECT generate_series('2020-09-01'::timestamp, CURRENT_DATE, '1 day'::interval) AS day
    ),
    ...
    supply AS (
        ...
        FROM calendar c
        LEFT JOIN cumulative_minted m ON m.day <= c.day AND c.day < m.day_of_next_change
    ),
    ...
    noncirc_supply AS (
        ...
        FROM calendar c
        LEFT JOIN cumulative_transfers t ON t.day <= c.day AND c.day < t.day_of_next_change
    )
