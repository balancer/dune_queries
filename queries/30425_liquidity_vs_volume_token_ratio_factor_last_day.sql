...
ratio_factors AS (
    SELECT
        pool_id AS pool,
        ROUND(
            SUM((2 * w_i * w_j /(w_i + w_j)) ^ 2) / SUM(w_i * w_j),
            { { Ratio factor decimals } }
        ) :: text AS ratio_factor
    FROM
        weights
    GROUP BY
        pool_id
...
liquidity_volume AS (
    SELECT
        ...
        r.ratio_factor :: text AS ratio_factor,
        liquidity,
        volume
    FROM
        liquidity l
...
last_liquidity_volume AS (
    SELECT
        *,
        CASE
            WHEN '{{Ratio factor}}' <> 'none' THEN (
                CASE
                    WHEN ratio_factor = '{{Ratio factor}}' THEN ratio_factor
                    ELSE 'Others'
                END
            )
            ELSE ratio_factor
