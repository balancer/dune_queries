WITH prices AS (
    ...
    daily_delta_balance_by_pool AS (
        SELECT day, pool, token, (cumulative_amount - LAG(cumulative_amount, 1) OVER (PARTITION BY pool, token ORDER BY day)) AS amount
        FROM balancer.view_balances b
    ),
    
    daily_delta_balance_by_token AS (
        SELECT day, token, SUM(amount) AS amount
        FROM daily_delta_balance_by_pool
        GROUP BY 1, 2
    ),
    ...
    total_value_locked AS (
        SELECT day, SUM(liquidity) AS amount
    ...
    daily_delta_usd_balance_by_token AS ( 
        SELECT b.day, b.token, COALESCE(amount /10 ^ t.decimals * p1.price, amount /10 ^ t.decimals * p2.price, 0) AS amount
        FROM daily_delta_balance_by_token b
        LEFT JOIN erc20.tokens t ON t.contract_address = b.token
        LEFT JOIN prices p1 ON p1.day = b.day AND p1.token = b.token
        LEFT JOIN dex_prices p2 ON p2.day = b.day AND p2.token = b.token
    ),
    ...
    weekly_delta_usd_balance AS (
        SELECT date_trunc('week', day) as monday_of_the_week, -- the Monday of the week of the state change
        SUM(amount) AS delta_balance
        FROM daily_delta_usd_balance_by_token
        GROUP BY 1
    ), 
    ...
    growth as (
        ...
        FROM TVL_at_end_of_sunday t
        -- join Sunday with the next Monday 
...
from growth initial_tvl_and_growth
inner join growth tvl_after_growth
on initial_tvl_and_growth.monday_of_the_week = tvl_after_growth.monday_of_the_week - INTERVAL '1 week'