WITH labels AS (
    ...
    transfers AS (
        ...
        WHERE evt_block_time <= '{{3. End date}}'
        UNION ALL
    ...
    daily_delta_bpt_by_pool AS (
        ...
        FROM exits e) foo
        WHERE ('{{1. Pool ID}}' = 'All'
        OR SUBSTRING(REGEXP_REPLACE('{{1. Pool ID}}', '^.', '\'),0,43)::bytea = pool)
        GROUP BY 1, 2, 3
    ),
...
SELECT * FROM cumulative_revenues WHERE week >= '{{2. Start date}}' AND week <= '{{3. End date}}'