...
last_liquidity_volume AS (
    SELECT
        ...
        f.swap_fee :: text,
        CASE
            WHEN swap_fee BETWEEN 0
            AND 0.1 THEN '0-0.1'
            WHEN swap_fee BETWEEN 0.1
            AND 0.25 THEN '0.1-0.25'
            WHEN swap_fee BETWEEN 0.25
            AND 0.5 THEN '0.25-0.50'
            WHEN swap_fee BETWEEN 0.5
            AND 1 THEN '0.50-1'
            WHEN swap_fee BETWEEN 1
            AND 5 THEN '1-5'
            WHEN swap_fee BETWEEN 5
            AND 10 THEN '5-10'
        END AS fee_class
    FROM
        liquidity_volume l
    ...
    WHERE
        DAY = date_trunc('day', CURRENT_DATE - '24h' :: INTERVAL)
)
SELECT
    *,
    CASE
        WHEN '{{Fee range}}' <> 'none' THEN (
            CASE
                WHEN fee_class = '{{Fee range}}' THEN fee_class
                ELSE 'Others'
            END
        )
        ELSE fee_class
