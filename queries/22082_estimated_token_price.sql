...
cumulative_amount_by_token AS (
    SELECT
        DAY,
        token,
        SUM(cumulative_amount / 10 ^ decimals) AS cumulative_amount
    FROM
        weekly_delta_balance_by_token b
        LEFT JOIN erc20.tokens t ON t.contract_address = b.token
    GROUP BY
        1,
        2
...
estimated_token_price AS (
    SELECT
        l.day,
        l.token,
        liquidity / cumulative_amount AS price
    FROM
        estimated_token_liquidity l
        INNER JOIN cumulative_amount_by_token c ON c.token = l.token
        AND c.day = l.day
)
INSERT INTO
    dune_user_generated.balancer_estimated_tokens_price
SELECT
    *
FROM
